...
| `com.example.sqlite-export.plist.template` | launchd plist template for hourly `--section user-profile` export. Copy to `~/Library/LaunchAgents/` after filling placeholders |
...
Setting `sqlite` does **not** affect the Session Log or Technical Learnings paths — those remain incremental/append-only.
